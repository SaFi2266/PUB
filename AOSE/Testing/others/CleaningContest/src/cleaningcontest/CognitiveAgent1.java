...
public class CognitiveAgent1 {
     public static String CA1="CA1"; 
   int face;
   public static int i;
   public static int j;
        ...
        switch(face)
        {
            case 0:
                if (moves%5==0 || i + 1 > 10 ||position[i+1][j].getText().equalsIgnoreCase("O")) {
                    ...
                    }else if ( i == CognitiveAgent2.i&&j==CognitiveAgent2.j)
                    {
                        score = (CognitiveAgent2.score+score)/2;
                         CognitiveAgent2.score=score;
                        System.out.println("Score devided between CA1 and CA2"+" CA1 = "+score);
                        Environment.position[i ][j].setText(agent);
                        Environment.position[i-1][j].setText("");
                    }else if ( i == ReactiveAgent1.i&&j==ReactiveAgent1.j)
                    {
                        ReactiveAgent1.score=score+ReactiveAgent1.score;
                        score=0;
                        System.out.println("CA1 lost his score RA1 score become "+ReactiveAgent1.score);
                        Environment.position[i ][j].setText(agent);
                        Environment.position[i-1][j].setText("");
                    }
                    else if ( i == ReactiveAgent2.i&&j==ReactiveAgent2.j)
                    {
                        ReactiveAgent2.score=score+ReactiveAgent2.score;
                        score=0;
                        System.out.println("CA1 lost his score RA2 score become "+ReactiveAgent2.score);
                        Environment.position[i ][j].setText(agent);
                        Environment.position[i-1][j].setText("");
                    }
                      else 
        ...
        if (Environment.objects==0)
        {
            for (int i=0;i<11;i++)
            {
                for(int j=0;j<11;j++)
                {
                    ...
                    {}
                    else if (position[i][j].getText().equalsIgnoreCase("")||position[i][j].getText().equalsIgnoreCase("CA1")||position[i][j].getText().equalsIgnoreCase("CA2")||position[i][j].getText().equalsIgnoreCase("RA1") ||position[i][j].getText().equalsIgnoreCase("RA2"))
                    {
                        Environment.position[i][j].setText("");
                    }
                }
             ...
             JOptionPane.showMessageDialog (null, "Results are CA1= "+CognitiveAgent1.score+" CA2= "+CognitiveAgent2.score +" RA1= "+ReactiveAgent1.score+" RA2= "+ReactiveAgent2.score);
            }
        }
        if(moves%1==0) 
        {
            JOptionPane.showMessageDialog (null, "Keep pressing Enter to see the agent move faster" );
        }
    
    }
   

    public CognitiveAgent1() {
        int l[] = new int[2];
            i = l[0];
            j = l[1];
            land = new String [11][11];       
    }
}
